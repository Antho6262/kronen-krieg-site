...
10. **Mode TV** (`pages/tv.html`, nouveau, absent de Volta) : dashboard plein écran type kiosque, pensé pour être affiché sur un écran/TV du local. Écran de démarrage (geste requis pour activer le son + plein écran) puis rotation automatique (13s) entre **4 slides** avec transition flip 3D + son "whoosh" synthétisé (Web Audio API, aucun fichier audio) : (1) Classement de la semaine (gains, barres animées, médailles top 3), (2) Stock/Arsenal (barres de stock par catégorie, alerte pulsante si sous le seuil), (3) Bilan de semaine (KPIs animés + mini-podium), (4) **Palmarès** (total actions tout temps, séries en tête actuelles, derniers badges débloqués). Fond animé de particules "braises" (canvas), grille tactique, filigrane croix de fer, glitch sur les titres. Contrôles clavier : **Espace** = pause/reprise (badge visuel, barre de progression figée), **← →** = navigation manuelle entre slides. Barre d'indices discrète en bas d'écran. Protégé par `requireSession()` (login requis) mais sans le shell/sidebar habituel (plein écran). Ajouté à `NAV_ITEMS` (js/app.js) et `PAGES_DISPO` (js/firebase-config.js) → gérable dans Admin → Permissions.
11. **Alerte Discord — quota atteint** (js/app.js : `verifierQuotaEtAlerter`, `envoyerWebhookQuota`) : appelée depuis `tracker.html` après chaque action **Réussite**. Vérifie le quota global du membre et ses quotas par catégorie (`quotas_categorie`), envoie une alerte une seule fois par semaine et par quota franchi via `config/discord_webhook_quota` (champ ajouté dans Admin → Config). Marqueurs anti-doublon stockés dans `semaines/{id}/quota_alertes/{membreId}/{global|catId}`.
12. **Badges & Records** (js/app.js : `BADGES_DEFS`, `majStatsEtBadges`) : à chaque verrouillage de semaine (auto ou manuel, `verrouillerSemaineAuto`/`bloquerSemaine`), calcule le classement de la semaine (actions réussies), incrémente `stats_membres/{id}` (`total_actions`, `semaines_gagnees`, `streak_actuel` — série de semaines consécutives terminées en tête, comparée via `config/dernier_gagnant_semaine`), et débloque les badges définis dans `BADGES_DEFS` (paliers d'actions cumulées : 50/100/250/500, paliers de série en tête : 2/3/5). Nouveaux badges annoncés sur le même webhook que les quotas (`config/discord_webhook_quota`). Affichés dans `profil.html` (panneau "Badges & records" du membre) et dans le slide "Palmarès" du mode TV.
13. **Recherche globale — Ctrl+F** (js/app.js : `injectGlobalSearch` et fonctions associées) : intercepte **Ctrl+F / Cmd+F** (remplace la recherche native du navigateur, demande explicite) sur toute page protégée pour ouvrir une modale de recherche (Échap pour fermer). Cherche dans les membres, les taxes (groupe/code), et les actions des 6 dernières semaines. Cliquer un résultat renvoie vers la page correspondante (admin/tracker/taxes) — pas de surlignage de ligne précise, juste la navigation.
14. **Journal d'audit** (js/app.js : `logAudit`, écrit dans `audit/{id}` = `{action, details, membre, membre_id, createdAt}`) : tracé sur les opérations sensibles — membres (ajout/modif/suppr/rôle/mdp/actif), semaines (créer/bloquer/débloquer/suppr), stock (catégories/produits créés/modifiés/supprimés), actions_dispo (créer/suppr), grades (créer/suppr), events, visibilité/permissions/config/taux par grade, actions du tracker (ajout/modif/suppr), taxes (payer/suppr), paiements (paye.html), blanchiments (effectuer/annuler). Consultable dans **Admin → Audit** (200 dernières entrées, trié du plus récent). Les éditions de quotas individuelles (`majQuotaAdmin`/`majQuotaCategorieAdmin`) ne sont volontairement pas tracées (trop fréquentes/peu sensibles) pour garder le journal lisible.

...
10. **Mode TV** (`pages/tv.html`, nouveau, absent de Volta) : dashboard plein écran type kiosque, pensé pour être affiché sur un écran/TV du local. Écran de démarrage (geste requis pour activer le son + plein écran) puis rotation automatique (13s) entre 3 slides avec transition flip 3D + son "whoosh" synthétisé (Web Audio API, aucun fichier audio) : (1) Classement de la semaine (gains, barres animées, médailles top 3), (2) Stock/Arsenal (barres de stock par catégorie, alerte pulsante si sous le seuil), (3) Bilan de semaine (KPIs animés + mini-podium). Fond animé de particules "braises" (canvas), grille tactique, filigrane croix de fer, glitch sur les titres. Contrôles clavier : **Espace** = pause/reprise (badge visuel, barre de progression figée), **← →** = navigation manuelle entre slides (réinitialise le minuteur de rotation). Barre d'indices discrète en bas d'écran. Protégé par `requireSession()` (login requis) mais sans le shell/sidebar habituel (plein écran). Ajouté à `NAV_ITEMS` (js/app.js) et `PAGES_DISPO` (js/firebase-config.js) → gérable dans Admin → Permissions.
